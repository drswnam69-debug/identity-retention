"""Figure 1: what the identity-retention fraction is, and why it is not tumor
purity adjustment. Panel (a) is a schematic. Panel (b) is real GSE14520 data for
...
paths = [((0.86, 0.14), C["co"], "purity change only",
         ...
         ((0.20, 0.84), C["id"], "dedifferentiation only",
          "identity adjustment removes it"),
         ((0.72, 0.68), C["bad"], "a real tumor", "both, in an unknown ratio")]
...
va = "bottom"
ax.text(0.905, 0.195, "purity change only\nthe identity covariate\ndoes not move",
        fontsize=7.2, color=C["co"], ha="right", va="bottom", linespacing=1.3)
ax.text(0.235, 0.845, "dedifferentiation only\nthe identity covariate\nmoves with it",
        fontsize=7.2, color=C["id"], ha="left", va="top", linespacing=1.3)
ax.text(0.755, 0.66, "a real tumor:\nboth, in an\nunknown ratio",
        fontsize=7.2, color=C["bad"], ha="left", va="top", linespacing=1.3)
ax.text(0.13, 1.02, "The two axes are not the same axis", fontsize=7.2,
        color=C["muted"], ha="left", va="bottom")

# ---------------------------------------------------------------- (b) real data
sys.path.insert(0, "/home/claude/rsi/code")
os.chdir("/home/claude/rsi")
def L(n, f):
...
ax.text(2.30, 2.35, f"axis trimmed at −3.25;\n{int((dD1 < -3.25).sum())} of "
        f"{len(dD1)} pairs lie to the left\nand are in every fit",
...
stem = "/home/claude/Figure1_concept"
