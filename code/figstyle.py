...
# GigaScience: full page width is 170 mm and the target is about 300 dpi at the
# final size. The figures are laid out at 6.85 in, which at 300 dpi prints 174 mm,
# 4 mm over. Rather than reflow every panel the night before submission, the
# raster metadata declares the resolution that puts the same pixels at exactly
# 170 mm. That raises the effective resolution rather than lowering it, and the
# SVG carries the same drawing as vector.
FULL_WIDTH_MM = 170.0


def save_all(fig, stem):
    """Write PNG and LZW TIFF at 170 mm full page width, plus SVG, all RGB."""
    ...
    im.save(f"{stem}.png", dpi=(dpi, dpi))
    im.save(f"{stem}.tiff", compression="tiff_lzw", dpi=(dpi, dpi))
