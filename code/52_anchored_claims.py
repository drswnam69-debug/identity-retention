...
def build_anchors():
    ...
    # ---- what the spread is not (§63, reported in Potential implications)
    SPR = J("SPREAD_IS_NOT_6v.json")
    _rs = SPR["retention_vs_absolute_shift"]
    add("ratio-scale within variance",
        r"mean within-signature variance of \*\*([\d.]+)\*\* against a total of 0\.14",
        SPR["ratio_scale_mean_within_variance"], tol=0.006)
    add("log between share", r"\*\*(\d+)%\*\* of the variance in log retention",
        SPR["between_signature_share"] * 100, tol=0.6)
    add("log between variance", r"sampling error \(([\d.]+) of 0\.514\)",
        SPR["between_signature_variance"])
    add("log total variance", r"sampling error \(0\.376 of ([\d.]+)\)",
        SPR["variance_of_log_point_estimates"])
    add("between-signature fold", r"standard deviation this implies is \*\*([\d.]+)-fold\*\*",
        SPR["between_signature_fold"], tol=0.006)
    _sp = SPR["spread_under_the_negative_control"]
    add("random pool spread fold", r"percentiles span \*\*([\d.]+)-fold\*\* against",
        _sp["matched_random"]["fold_p05_p95"], tol=0.06)
    add("D1 spread fold", r"against \*\*([\d.]+)-fold\*\* under D1",
        _sp["D1"]["fold_p05_p95"], tol=0.06)
    add("random pool log variance",
        r"variance of log retention is \*\*([\d.]+)\*\* against 0\.524",
        _sp["matched_random"]["variance_of_log"])
    add("D1 log variance", r"is \*\*0\.667\*\* against (\d+\.\d+)\.",
        _sp["D1"]["variance_of_log"])
    add("source study eta2 ratio",
        r"source study accounts for (\d+)% of the variance on the ratio scale",
        SPR["source_study_effect_ratio"]["eta_squared"] * 100, tol=0.6)
    add("source study eta2 log", r"it accounts for (\d+)% against the 27%",
        SPR["source_study_effect_log"]["eta_squared"] * 100, tol=0.6)
    add("source study null eta2", r"against the (\d+)% that 33 groups would give by chance",
        SPR["source_study_effect_log"]["eta_squared_expected_under_null"] * 100, tol=0.6)
    add("retention vs shift all", r"retention is negatively rank-correlated with the absolute size of the shift it is a fraction of, ρ = −([\d.]+)",
        abs(_rs["all"]["spearman_rho"]))
    add("retention vs shift rising", r"at ρ = \+([\d.]+) among the 60\s*\n?\s*rising sets",
        _rs["rising"]["spearman_rho"])
    add("retention vs shift falling", r"and ρ = −([\d.]+) among the 59 falling ones",
        abs(_rs["falling"]["spearman_rho"]))
    ...
    # a bare "rho = +x (P = 0.36)" also matches an unrelated correlation elsewhere,
    # so the anchor carries the words that make it this one
    add("mRNA-protein rho",
        r"Across the 97 signatures evaluable at both levels, \*\*ρ = \+([\d.]+) \(\*P\* = 0\.36\)\*\*",
        MR["spearman_rho"])
    ...
    # ---- Table 3 and the archive gaps
    g = GAP["GSE14520"]["per_gene"]
    add("Table 3 MTARC2 GSE14520", r"\| \*MTARC2\* \| −([\d.]+) \(1\.2 × 10⁻²⁴\)", abs(g["MTARC2"]["paired_mean_delta"]))
    add("Table 3 POR GSE14520", r"\| \*POR\* \| −([\d.]+) \(8\.8 × 10⁻²³\)", abs(g["POR"]["paired_mean_delta"]))
    add("Table 3 CYB5R1 GSE14520", r"\| \*CYB5R1\* \| \+([\d.]+) \(3\.3 × 10⁻²⁷\)", g["CYB5R1"]["paired_mean_delta"])
    add("Table 3 NQO1 GSE14520", r"\| \*NQO1\* \| \+([\d.]+) \(5\.4 × 10⁻²⁰\)", g["NQO1"]["paired_mean_delta"])
...
if __name__ == "__main__":
    # Analyses reported in full in the supplementary note are part of the same
    # checked corpus; running against the manuscript alone would report every
    # relocated claim as a lost anchor rather than checking it.
    txt = open(f"{IR_DOCS}/manuscript_v4.md", encoding="utf-8").read()
    _note = f"{IR_DOCS}/SupplementaryNote_v1.md"
