...
def main() -> int:
    ...
    for name in ("GOBP_NEGATIVE_REGULATION_OF_FERROPTOSIS",
        ...
        if len(present) < 5:
            # Distinguish "the set is not in the archive" from "the set has too
            # few members on this platform". Both produced a zero here, and a
            # pre-submission reader took the second reading from the first
            # cause and concluded the omission was a property of the data. It
            # is not: these are Gene Ontology sets and the deposited GMT is the
            # liver C2:CGP collection, so the membership was never loaded.
            loaded = bool(members)
            out["h6_GSE76427"]["sets"][name] = {
                "n_members_after_overlap_removal": len(members),
                "n_on_platform": len(present),
                "membership_loaded": loaded,
                "status": ("fewer than 5 members on this platform" if loaded else
                           "NOT ATTEMPTED: this set's membership is not in the "
                           "deposited gene-set files, so nothing was scored and "
                           "the zero above is the absence of the file, not a "
                           "property of the cohort")}
            print(f"  H6 {name[:44]:46s} "
                  f"{'membership not in archive -- not attempted' if not loaded else str(len(present)) + ' on platform -- skipped'}")
            continue
