...
for name, genes, ec, fc, x, wt in mods:
    axA.add_patch(FancyBboxPatch((x - 0.166, 0.548), 0.332, 0.389,
                                 boxstyle="round,pad=0,rounding_size=0.02",
                                 lw=1.2, edgecolor=ec, facecolor=fc))
    axA.text(x, 0.895, name, ha="center", fontsize=7.2, fontweight="bold", color=ec)
    wrapped, line = [], ""
    for g in genes:
        if line and len(line) + 2 + len(g) > 14:
            wrapped.append(line); line = g
        else:
            line = (line + ", " + g) if line else g
    wrapped.append(line)
    axA.text(x, 0.735, "\n".join(wrapped), ha="center", va="center",
             fontsize=7.2, family="monospace", color=F.C["ink"], linespacing=1.45)
    axA.annotate("", xy=(x, 0.475), xytext=(x, 0.545),
                 arrowprops=dict(arrowstyle="-|>", lw=1.2, color=ec, mutation_scale=7))
    axA.text(x + 0.048, 0.505, wt, fontsize=7.2, color=ec, fontweight="bold")
axA.plot([0.175, 0.825], [0.455, 0.455], color=F.C["rule"], lw=1.0)
axA.annotate("", xy=(0.5, 0.378), xytext=(0.5, 0.450),
             arrowprops=dict(arrowstyle="-|>", lw=1.2, color=F.C["ink"], mutation_scale=7))
axA.add_patch(FancyBboxPatch((0.145, 0.255), 0.71, 0.115,
                             boxstyle="round,pad=0,rounding_size=0.02",
                             lw=1.3, edgecolor=F.C["ink"], facecolor="white"))
axA.text(0.5, 0.312, "composite  =  ½ (SUPPLY + REDUCTION)  −  DRAIN",
         ha="center", va="center", fontsize=7.6, fontweight="bold")
axA.text(0.5, 0.112, "each module = mean of within-cohort gene-wise z-scores\n"
                     "locked 25 Aug 2026 · SHA-256 7a2bf934…b680046\n"
                     "POR was locked into DRAIN on a false premise (§6l); removing it\n"
                     "changes no direction and no verdict in the two array cohorts",
         ha="center", va="center", fontsize=7.2, color=F.C["muted"], linespacing=1.6)
...
for y, (n, nn, r, lo, hi) in zip(ys, coh):
    ...
    axB.text(0.60, y, f"n = {nn}", fontsize=7.2, va="center", color=F.C["muted"])
yp = -1.05
...
axB.text(0.60, yp, f"n = {pooled[1]}", fontsize=7.2, va="center", fontweight="bold")
axB.axhline(-0.5, color=F.C["rule"], lw=0.7)
axB.axvline(0, color=F.C["ink"], lw=0.8)
axB.set_yticks(list(ys) + [yp])
axB.set_yticklabels([c[0] for c in coh] + ["Pooled  (I² = 0%)"], fontsize=7.2)
axB.set_xlim(-0.05, 0.72); axB.set_ylim(-1.7, len(coh) - 0.3)
axB.set_xlabel("Spearman ρ, index vs histological severity", fontsize=7.2)
...
for i, (dv, lab) in enumerate(((diff76, "GSE76427\n52 pairs"), (diff14, "GSE14520\n213 pairs"))):
    ...
    lx, ha = ((i - 0.30, "right") if i == 0 else (i + 0.30, "left"))
    axA.text(lx, np.median(dv), f"median {np.median(dv):+.2f}", fontsize=7.2,
             va="center", ha=ha, fontweight="bold")
    axA.text(i, -3.05, f"{(dv > 0).mean() * 100:.0f}% of pairs positive",
             ha="center", fontsize=7.2, color=F.C["muted"])
axA.axhline(0, color=F.C["ink"], lw=0.9)
axA.set_xticks([0, 1]); axA.set_xticklabels(["GSE76427\n52 pairs", "GSE14520\n213 pairs"], fontsize=7.2)
axA.set_xlim(-0.95, 1.80); axA.set_ylim(-3.3, 4.4)
axA.set_ylabel("Δ composite index, tumor − paired adjacent liver", fontsize=7.2)
...
for i, (lab, v) in enumerate(vals):
    ...
    axB.text(i, v + 0.03, f"{v:+.2f}", ha="center", fontsize=7.2, fontweight="bold",
             color=F.C["protect"])
axB.set_xticks([0, 1]); axB.set_xticklabels([v[0] for v in vals], fontsize=7.2)
axB.set_ylim(0, 1.02); axB.set_ylabel("Cliff's δ", fontsize=7.2)
axB.set_title("Effect size", fontsize=7.4, fontweight="bold", loc="left", pad=4)
for s_ in ("top", "right"): axB.spines[s_].set_visible(False)
axB.tick_params(labelsize=6.2, length=2.5, width=0.6)

axC = F.blank_axes(fig, [0.775, 0.175, 0.220, 0.645])
fig.text(0.742, 0.945, "c", **LB)
axC.text(0.0, 1.0, "GSE164760", fontsize=7.2, fontweight="bold", va="top")
axC.text(0.0, 0.86, "53 MASH-HCC tumors vs\n29 non-tumoral MASH livers\n"
                    "adjacent to HCC; patient\npairing not declared",
         fontsize=7.2, va="top", color=F.C["muted"], linespacing=1.6)
axC.text(0.0, 0.50, "δ = +0.10\nP = 0.44", fontsize=7.6, va="top", fontweight="bold",
         color=F.C["patho"], linespacing=1.5)
axC.text(0.0, 0.245, "Not supported. The\ncomparator is already-\ndiseased liver, and this\n"
                     "cohort is reported as\nuninformative for every\ndownstream adjustment\nby a rule fixed in advance.",
         fontsize=7.2, va="top", color=F.C["ink"], linespacing=1.6)
